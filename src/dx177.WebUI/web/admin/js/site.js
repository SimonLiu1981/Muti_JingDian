...
var flinkboxgao;
var flinkboxgao = flinkbox.offsetHeight;
var flinkoMarquee = document.getElementById("flinkdiv"); //滚动对象
var flinloLineHeight = 30; //单行高度，像素
var flinkLineCount = flinkboxgao/3; //实际行数
var flinkScrollAmount = 5; //每次滚动高度，像素
function flinkrun() {
flinkoMarquee.scrollTop += flinkScrollAmount;
if (flinkoMarquee.scrollTop >= flinkboxgao)
flinkoMarquee.scrollTop = 0;
if ( flinkoMarquee.scrollTop % flinloLineHeight == 0 ) {
window.setTimeout( "flinkrun()", 4000 );
} else {
window.setTimeout( "flinkrun()", 50 );
}
}
flinkoMarquee.innerHTML += flinkoMarquee.innerHTML;
window.setTimeout( "flinkrun()", 4000 );