<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WapUserList.aspx.cs" Inherits="dx177.WebUI.admin.UserMgnt.WapUserList" %>

 <%@ Register TagPrefix="cc1" Namespace="COM.ZTE.TPG.App.UI.WebForm" Assembly="COM.ZTE.TPG.App.UI.WebForm" %>

<%@ Register Assembly="My97" Namespace="My97" TagPrefix="cc2" %>
 <!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" >
<html>
<head>
    <title>酒店管理</title>
    <meta content="Microsoft Visual Studio .NET 7.1" name="GENERATOR">
    <meta content="C#" name="CODE_LANGUAGE">
    <meta content="JavaScript" name="vs_defaultClientScript">
    <meta content="http://schemas.microsoft.com/intellisense/ie5" name="vs_targetSchema">
    <link href="../css/cssCn.css" type="text/css" rel="stylesheet">
    <script src="../../js/jquery-1.3.2.min.js" type="text/javascript"></script>
    <link href="/Plugin/thickbox/thickbox.css" rel="stylesheet" type="text/css" />
    <script src="/Plugin/thickbox/thickbox.js" type="text/javascript"></script>
    <script>
    
        function EditBook(bookSeqno) {
            var url = "/admin/UserMgnt/EditWapUser.aspx?Seqno=" + bookSeqno + "TB_iframe=true&height=500&width=800&modal=true";
            ShowPopup(url);
            return false;
        }

        function RefeshBook() {
            $('#btnSearch').click();
        }        
        
    
    </script>
</head>
<body ms_positioning="GridLayout">
    <form id="Form1" method="post" runat="server">
    <table cellspacing="0" cellpadding="0" width="100%" border="0">
        <tr>
            <td>
                <div class="div_subtitle">
                    当前位置：手机分销管理<span class="arrow_subtitle">&gt;</span>手机分销表</div>
            </td>
        </tr>
    </table>
    <table id="tb_content" cellspacing="0" cellpadding="0" align="center">
        <tbody>
            <tr>
                <td>
                    <table class="tb_searchbar" cellspacing="1" width="100%">
                        <tbody>
                            <tr>
                                <td class="td_head">
                                    &nbsp;请填写查询条件
                                </td>
                            </tr>
                            <tr>
                                <td align="right">
                                    <asp:Button ID="btnSearch" runat="server" Text="查询" CssClass="Button" OnClick="btnSearch_Click">
                                    </asp:Button><font face="宋体">&nbsp;</font> <font face="宋体">&nbsp;</font>
                              
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    
                    
                    <cc1:ZteGrid ID="dgWapUserlist" runat="server" Style="margin-top: 5px" CssClass="tb_datalist1"
                        PageChange="TextBox" Width="100%" AllowPaging="True" 
                        PageLanguage="Chinese" IsAllowPaging="True"
                        AutoGenerateColumns="False" CacheData="ServerCach" SortMode="Total" PageCSS="scrollButton"
                        BorderStyle="Solid" OnItemDataBound="dgWapUserlist_ItemDataBound" 
                        OnItemCommand="dgWapUserlist_ItemCommand">
                        <ItemStyle CssClass="tr_datarow" />
                        <AlternatingItemStyle CssClass="tr_alertrow"></AlternatingItemStyle>
                        <Columns>
                            <asp:BoundColumn Visible="False" DataField="Seqno" HeaderText="Seqno"></asp:BoundColumn> 
                            <asp:BoundColumn DataField="UserName" HeaderText="用户名"></asp:BoundColumn>
                            <asp:BoundColumn DataField="Name" HeaderText="名称"></asp:BoundColumn>
                            <asp:BoundColumn DataField="Mobile" HeaderText="手机"></asp:BoundColumn>
                            <asp:BoundColumn DataField="Phone" HeaderText="电话"></asp:BoundColumn>
                            <asp:BoundColumn DataField="domainName" HeaderText="域名"></asp:BoundColumn>    
                            <asp:BoundColumn DataField="domaintype" HeaderText="域名类型"></asp:BoundColumn>    
                            <asp:TemplateColumn>
                                <HeaderStyle Width="80px" />
                                <HeaderTemplate>
                                    操作
                                </HeaderTemplate>
                                <ItemTemplate>    
                                <a href="#" onclick="return EditBook('<%#DataBinder.Eval(Container.DataItem, "Seqno") %>');" > 编辑</a>
                                                                                                
                                   
                                    
                                </ItemTemplate>
                            </asp:TemplateColumn>
                        </Columns>
                        <HeaderStyle CssClass="tr_title"></HeaderStyle>
                        <SelectedItemStyle CssClass="tr_selected"></SelectedItemStyle>
                        <PagerStyle CssClass="tr_pagenumber"></PagerStyle>
                    </cc1:ZteGrid>
                </td>
            </tr>
        </tbody>
    </table>
    </form>
</body>
