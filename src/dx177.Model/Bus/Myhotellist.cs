...
namespace dx177.Model.Bus
{
		...
		[Serializable]
		public class Myhotellist : BaseModel
		{
		#region 自动生成代码
			...
			{
				if (obj != null && obj is Myhotellist)
				{
					if (obj == this)
						return true;
					Myhotellist castObj = (Myhotellist)obj; 
					...
					if (!this.m_Showindex.Equals(castObj.Showindex))
						return false;
			...
			public override int GetHashCode()
			{
					...
					hash = hash <<  8;
					if ((object)m_Showindex != null)
					{
						hash = hash ^ m_Showindex.GetHashCode();
					}
			...
			public Myhotellist GetOldValue()
			{
				return OldValue as Myhotellist;
			}
		...
		#region Showindex属性
			...
			public int Showindex
			{
				...
				set
					{
						if (m_Showindex as object == null || !m_Showindex.Equals(value))
						{
							m_Showindex = value;
							MarkUpdated();
						}						
		...
		#region static CreateDataTable/FillDataRow/DataRow2Obj/Dt2Objs/FillDataTable
			...
			static public DataTable CreateDataTable(string tableName,ColumnNameEnum cne)
			{
				...
				if (cne == ColumnNameEnum.DBName)
				{
					...
					dtResult.Columns.Add(new DataColumn("showindex",typeof(int)));
				}
			...
			/// <summary>
			/// 将DataRow转换成Myhotellist对象
			/// </summary>
			/// <param name="dr">由CreateTable生成的DataTable通过NewRow得到的DataRow对象</param>
			/// <param name="cne">dr的列名映射方式:DB列名或属性名</param>
			/// <returns>Myhotellist对象</returns>
			public static Myhotellist Dr2Obj(DataRow dr,ColumnNameEnum cne)
			{
				if (dr == null)
				{
					throw new ArgumentNullException("dr");
				}
				Myhotellist obj = new Myhotellist();
				if (ColumnNameEnum.DBName == cne)
				{
					...
					obj.Showindex = (dr["showindex"] == Convert.DBNull) ? 0  : (int)Convert.ChangeType(dr["showindex"],typeof(int));
				}
				else if (ColumnNameEnum.PropertyName == cne)
				{
					obj.Seqno = (dr["Seqno"] == Convert.DBNull) ? 0  : (int)Convert.ChangeType(dr["Seqno"],typeof(int));
			...
			/// <summary>
			/// 将DataRow转换成Myhotellist对象(默认列名映射为属性名)
			/// </summary>
			/// <returns>Myhotellist对象</returns>
			public static Myhotellist Dr2Obj(DataRow dr)
			{
				return Dr2Obj(dr,ColumnNameEnum.DBName);
			}
			
			/// <summary>
			/// 将DataTabe转换成的Myhotellist对象集合
			/// </summary>
			/// <param name="dt">由CreateTable生成的DataTable</param>
			/// <param name="cne">dt的列名映射方式:DB列名或属性名</param>
			/// <returns>Myhotellist对象的集合</returns>
			public static List<Myhotellist> Dt2Objs(DataTable dt,ColumnNameEnum cne)
			{
				if (dt == null)
				{
				...
				List<Myhotellist> alResult = new List<Myhotellist>();
			...
			/// <summary>
			/// 将DataTabe转换成的Myhotellist对象集合(默认列名映射为属性名)
			/// </summary>
			/// <param name="dt">由CreateTable生成的DataTable</param>
			/// <returns>Myhotellist对象的集合</returns>
			public static  List<Myhotellist> Dt2Objs(DataTable dt)
			{
				return Dt2Objs(dt, ColumnNameEnum.DBName);
			}
			
			/// <summary>
			/// 用objs的Myhotellist对象集合填充DataTable
			/// </summary>
			/// <param name="dt">由CreateTable生成的DataTable</param>
			/// <param name="objs">Myhotellist对象集合</param>
			/// <param name="cne">dt的列名映射方式:DB列名或属性名</param>
			public static void FillDataTable(DataTable dt,IList objs,ColumnNameEnum cne)
			{
				...
				foreach(Myhotellist obj in objs)
				{
			...
			/// <summary>
			/// 用objs的Myhotellist对象集合填充DataTable(默认列名映射为属性名)
			/// </summary>
			/// <param name="dt">由CreateTable生成的DataTable</param>
			/// <param name="objs">Myhotellist对象集合</param>
		...
		{
			Myhotellist old = this.OldValue as Myhotellist;
			if (old != null)
			{
