	...
	public sealed class RewriteActionParser : RewriteActionParserBase
	{
		...
		public override IRewriteAction Parse(XmlNode node, object config)
		{
			...
			if (xmlNode2 != null)
			{
				...
				{
					if (xmlNode2.Value == "stop")
					{
						processing = RewriteProcessing.StopProcessing;
					}
					else
					{
						if (xmlNode2.Value != "continue")
						{
							throw new ConfigurationErrorsException(MessageProvider.FormatString(Message.ValueOfProcessingAttribute, new object[]
							{
								xmlNode2.Value,
								"continue",
								"restart",
